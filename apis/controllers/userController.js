    ...
    if (checkMail) {
      ...
      if (
        ...
        await SaveData.save().then((result) => {
          res
            .status(200)
            .json({ message: "Registered Successfully", Data: result });
        });
      }
    }
  } catch (err) {
    ...
    await vendors.findByIdAndUpdate(
      ...
      (err, result) => {
        if (!err) {
          ...
          res
            .status(201)
            .json({ message: "Something error in the updation", err });
        }
      }
    );
  } catch (err) {
...
async function delete_user(req, res){
   try{
    ...
    userModel.findOneAndDelete({_id:user_id}, (result, err)=>{
       if(err){
           res.status(201).json(err)
       }else{
...
async function delete_vendor(req, res){
    ...
    vendors.findOneAndDelete({_id:vendor_id}, (err, response)=>{
        if(!err){
